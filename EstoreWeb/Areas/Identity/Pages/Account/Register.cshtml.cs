...
namespace EstoreWeb.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        {
            if (_roleManager.RoleExistsAsync(SD.RoleCustomer).GetAwaiter().GetResult() == false)
            {
                await _roleManager.CreateAsync(new IdentityRole() { Name = SD.RoleCustomer });
            ...
            Input = new()
            {
                Roles = _roleManager.Roles.Select(r => r.Name).Select(v => new SelectListItem()
                {
                    Text = v,
                    Value = v
                })
